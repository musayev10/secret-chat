...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; user-select: none; }
        ...
        .btn-circle { background: #0a84ff; border: none; color: #fff; width: 36px; height: 36px; border-radius: 18px; font-size: 16px; display: flex; align-items: center; justify-content: center; cursor: pointer; }
...
<body>
    ...
    <div id="chat-screen">
        ...
        <div class="input-bar">
            ...
            <button class="btn-circle" id="voiceBtn" onmousedown="startVoiceRecord()" onmouseup="stopVoiceRecord()" ontouchstart="startVoiceRecord()" ontouchend="stopVoiceRecord()">🎤</button>
    ...
    <script>
        let inputStr = '';
        const PIN = '20092010';
        const SERVER_URL = "wss://secret-chat-n5po.onrender.com/ws?device_key=key_faye_phone_7730";
        ...
        function connectWS() {
            ...
            ws.onmessage = async (e) => {
                ...
                if (data.type === 'video') addMsg(`<video src="${data.src}" controls></video>`, 'their', true);
                if (data.type === 'audio') addMsg(`<audio src="${data.src}" controls></audio>`, 'their', true);
                
                // WebRTC Сигналы Звонков
        ...
        function sendMedia(input) {
            ...
            reader.onload = (e) => {
                ...
                const tag = type === 'image' ? `<img src="${e.target.result}">` : `<video src="${e.target.result}" controls></video>`;
                addMsg(tag, 'my', true);
            };
        ...
        async function startVoiceRecord() {
            try {
                ...
                mediaRecorder.start();
                document.getElementById('voiceBtn').style.background = '#ff3b30';
            } catch(e) { alert("Микрофон недоступен"); }
        }

        function stopVoiceRecord() {
            if (!mediaRecorder || mediaRecorder.state === 'inactive') return;
            document.getElementById('voiceBtn').style.background = '#0a84ff';
            mediaRecorder.stop();
            mediaRecorder.onstop = () => {
                const blob = new Blob(audioChunks, { type: 'audio/webm' });
                const reader = new FileReader();
                reader.onload = (e) => {
                    ws.send(JSON.stringify({ type: 'audio', src: e.target.result }));
                    addMsg(`<audio src="${e.target.result}" controls></audio>`, 'my', true);
                };
